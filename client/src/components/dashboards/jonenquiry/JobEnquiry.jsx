...
const JobEnquiry = () => {
  ...
  //   [d.visitorId?.name, d.jobRole].some((v) =>
  ...
  const filtered = data
    ...
    .filter((d) => {
      ...
      const checkInTime = d.visitorId?.checkInTime;
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-medium">Job Enquiries</h2>
          ...
          <input
            ...
            className="text-xs px-3 py-1.5 border border-gray-200 rounded-lg bg-gray-50 outline-none w-48 placeholder-gray-400"
          />
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 text-left">
              {["Visitor", "Phone", "Check-in", "Job role", "Action"].map((h) => (
                <th key={h} className="px-5 py-3 text-[11px] font-medium uppercase tracking-wider text-gray-400">
                  {h}
                </th>
              ))}
            </tr>
                ...
                <tr key={item._id} className="border-t border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-8 h-8 rounded-full ${c.bg} ${c.text} flex items-center justify-center text-xs font-medium shrink-0`}>
                        {initials(v.name)}
                      </div>
                  ...
                  {/* Phone */}
                  <td className="px-5 py-3 text-xs text-gray-500">{v.phone}</td>

                  {/* Check-in */}
                  <td className="px-5 py-3 text-xs text-gray-500">{fmtDate(v.checkInTime)}</td>

                  {/* Job Role */}
                  <td className="px-5 py-3">
                    <span className="text-[11px] font-medium bg-blue-50 text-blue-800 px-2.5 py-1 rounded-full">
                      {item.jobRole}
                    </span>
                  </td>
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
          ...
          <div className="flex gap-2">
            <button
              ...
              disabled={page === 1}
              className="text-xs px-3 py-1.5 border border-gray-200 rounded-lg disabled:opacity-40 hover:bg-gray-50 transition-colors cursor-pointer"
            >
              ← Prev
            </button>
            <span className="text-xs px-3 py-1.5 bg-gray-900 text-white rounded-lg">{page}</span>
            <button
