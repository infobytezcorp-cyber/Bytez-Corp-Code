...
const EnquiryListContent = () => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      bySource: {
        'Website': uniqueClients.filter(e => e.source === 'Website').length,
        'Telecaller': uniqueClients.filter(e => e.source === 'Telecaller').length,
        'Tawk.to': uniqueClients.filter(e => e.source === 'Tawk.to').length,
        'Referral': uniqueClients.filter(e => e.source === 'Referral').length,
      },
  ...
  const filteredEnquiries = useMemo(() => {
    ...
    const filtered = enquiries.filter(enquiry => {
      ...
      return matchStage && matchSource && matchSearch && matchCareType;
    });
                    ...
                    <table className="w-full">
                      <thead className="border-b border-gray-200 bg-gray-50">
                        <tr>
                          ...
                          <th className="px-6 py-4 text-left text-xs font-semibold text-gray-700 uppercase tracking-wider">Aadhaar</th>
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {filteredEnquiries.map((enquiry) => (
                          <tr key={enquiry.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <p className="text-sm text-gray-900">{enquiry.aadhaar || '-'}</p>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
